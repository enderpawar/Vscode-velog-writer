...
export function activate(context: vscode.ExtensionContext) {
    ...
    try {
        ...
        context.subscriptions.push(vscode.window.registerWebviewViewProvider(VelogWebviewProvider.viewType, provider));
    } catch (error) {
        ...
        try {
            ...
            await vscode.window.withProgress(
                ...
                async progress => {
                    ...
                    // 2. 필터 옵션
                    const useFilter = await vscode.window.showQuickPick(['전체 커밋', '특정 경로만 필터링'], {
                        placeHolder: '커밋 범위를 선택하세요',
                    });

                    const options: GitCommitOptions = { includeFiles: true };
                    ...
                    // 4. 통계 정보 포함 여부
                    const includeStats = await vscode.window.showQuickPick(['네, 통계 포함', '아니요, 글만 작성'], {
                    ...
                    // 자동 저장 옵션
                    const autoSave = await vscode.window.showQuickPick(['에디터에만 보기', '파일로 저장'], {
    ...
    const viewStatsCommand = vscode.commands.registerCommand('velog-auto-writer.viewStats', async () => {
        try {
            ...
            await vscode.window.withProgress(
                ...
                async progress => {
                    const commits = await getGitCommits(workspaceFolder.uri.fsPath, Number(daysInput), {
                        includeFiles: true,
                    });
